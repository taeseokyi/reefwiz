...
// ============================================================
int tp=0, tf=0;
std::string cmd_out(const char* c) { flushOutput(); injectSerial(c); handleCommand(); return flushOutput(); }
void ok(bool c, const char* n) { if(c){printf("  [PASS] %s\n",n);tp++;}else{printf("  [FAIL] %s\n",n);tf++;} }

int main() {
    printf("===== AquaWiz Firmware Simulation Test =====\n\n");
    setup(); flushOutput();
    for(int i=0;i<64;i++){advanceMillis(130);loop();}
    flushOutput(); currentMode=MODE_IDLE;

    printf("--- help ---\n");
    auto o=cmd_out("help");
    ok(o.find("명령어")!=std::string::npos, "help 출력");

    printf("--- settime ---\n");
    o=cmd_out("settime:14"); ok(o.find("[OK]")!=std::string::npos,"settime:14 OK");
    o=cmd_out("settime:25"); ok(o.find("[ERR]")!=std::string::npos,"settime:25 ERR");

    printf("--- setref ---\n");
    o=cmd_out("setref:8.5"); ok(o.find("[OK]")!=std::string::npos,"setref:8.5 OK");
    o=cmd_out("setref:0.1"); ok(o.find("[ERR]")!=std::string::npos,"setref:0.1 ERR");
    o=cmd_out("setref:35"); ok(o.find("[ERR]")!=std::string::npos,"setref:35 ERR");

    printf("--- settemp ---\n");
    o=cmd_out("settemp:-0.3"); ok(o.find("[OK]")!=std::string::npos,"settemp:-0.3 OK");
    o=cmd_out("settemp:15"); ok(o.find("[ERR]")!=std::string::npos,"settemp:15 ERR");

    printf("--- motor pins ---\n");
    o=cmd_out("m1f:10");
    ok(o.find("정방향")!=std::string::npos,"m1f 정방향");
    ok(pinStates[4]==HIGH,"M1_IN1=HIGH"); ok(pinStates[5]==LOW,"M1_IN2=LOW");
    cmd_out("m1s");
    ok(pinStates[4]==LOW,"m1s→IN1=LOW"); ok(pinStates[5]==LOW,"m1s→IN2=LOW");

    o=cmd_out("m2b:5");
    ok(o.find("역방향")!=std::string::npos,"m2b 역방향");
    ok(pinStates[6]==LOW,"M2_IN3=LOW(rev)"); ok(pinStates[7]==HIGH,"M2_IN4=HIGH(rev)");
    cmd_out("m2s");

    o=cmd_out("m3f:5");
    ok(pinStates[8]==HIGH,"M3_IN1=HIGH"); ok(pinStates[9]==LOW,"M3_IN2=LOW");
    cmd_out("m3s");

    o=cmd_out("m4b:5");
    ok(pinStates[10]==LOW,"M4_IN3=LOW(rev)"); ok(pinStates[11]==HIGH,"M4_IN4=HIGH(rev)");
    cmd_out("m4s");

    o=cmd_out("m1f:0"); ok(o.find("[ERR]")!=std::string::npos,"m1f:0 ERR");
    o=cmd_out("m1f:5000"); ok(o.find("[ERR]")!=std::string::npos,"m1f:5000 ERR");

    printf("--- motor timer ---\n");
    cmd_out("m3f:2"); ok(pinStates[8]==HIGH,"M3 시작");
    advanceMillis(1000); loop(); ok(pinStates[8]==HIGH,"M3 1초후 동작");
    advanceMillis(1500); loop(); ok(pinStates[8]==LOW,"M3 2.5초후 정지");

    printf("--- air solenoid ---\n");
    o=cmd_out("air:10:2");
    ok(o.find("참조ON")!=std::string::npos,"air 참조ON");
    ok(pinStates[12]==HIGH,"SOL_REF=HIGH"); ok(pinStates[13]==LOW,"SOL_TANK=LOW");
    advanceMillis(2500); flushOutput(); loop(); o=flushOutput();
    ok(o.find("수조ON")!=std::string::npos,"2.5s→수조ON 전환");
    ok(pinStates[12]==LOW,"SOL_REF=LOW"); ok(pinStates[13]==HIGH,"SOL_TANK=HIGH");
    cmd_out("airoff");
    ok(pinStates[12]==LOW&&pinStates[13]==LOW,"airoff 양쪽 OFF");

    printf("--- air error ---\n");
    o=cmd_out("air:0:5"); ok(o.find("[ERR]")!=std::string::npos,"air:0 ERR");
    o=cmd_out("air:10:0"); ok(o.find("[ERR]")!=std::string::npos,"air:10:0 ERR");

    printf("--- wait ---\n");
    o=cmd_out("wait:3"); ok(o.find("[대기]")!=std::string::npos,"wait:3 OK");
    o=cmd_out("wait:0"); ok(o.find("[ERR]")!=std::string::npos,"wait:0 ERR");

    printf("--- buffer overflow ---\n");
    char big[200]; memset(big,'a',140); big[140]=0;
    o=cmd_out(big);
    ok(o.find("[ERR]")!=std::string::npos,"140자 거부");
    ok(o.find("초과")!=std::string::npos,"초과 메시지");

    printf("--- seq ---\n");
    o=cmd_out("seq:m1f:1|m1s");
    ok(o.find("[SEQ] 시작")!=std::string::npos,"seq 시작");
    ok(o.find("2단계")!=std::string::npos,"2단계 표시");
    for(int i=0;i<200;i++){advanceMillis(50);loop();}
    o=flushOutput();
    ok(o.find("[SEQ] 완료")!=std::string::npos,"seq 완료");

    printf("--- seq overflow ---\n");
    std::string s="seq:a";
    for(int i=1;i<21;i++) s+="|a";
    o=cmd_out(s.c_str());
    ok(o.find("[ERR]")!=std::string::npos,"21단계 거부");
    ok(o.find("20")!=std::string::npos,"20단계 표시");

    printf("--- seq running ---\n");
    cmd_out("seq:m1f:30|m2f:30");
    o=cmd_out("seq:m3f:5");
    ok(o.find("실행중")!=std::string::npos,"실행중 거부");
    cmd_out("seqstop");

    printf("--- seqstop ---\n");
    cmd_out("seq:m1f:30");
    o=cmd_out("seqstop");
    ok(o.find("중단")!=std::string::npos,"seqstop 중단");
    ok(pinStates[4]==LOW,"M1 OFF"); ok(pinStates[12]==LOW,"SOL OFF");

    printf("--- unknown ---\n");
    o=cmd_out("xyz");
    ok(o.find("[?]")!=std::string::npos,"알 수 없는 명령");

    printf("--- case insensitive ---\n");
    o=cmd_out("HELP"); ok(o.find("명령어")!=std::string::npos,"HELP→help");
    o=cmd_out("M1F:3"); ok(o.find("정방향")!=std::string::npos,"M1F→m1f");
    cmd_out("m1s");

    printf("--- status ---\n");
    o=cmd_out("status");
    ok(o.find("상태")!=std::string::npos,"status 출력");
    ok(o.find("M1")!=std::string::npos,"M1 상태");

    printf("--- resetref ---\n");
    cmd_out("setref:8.5");
    o=cmd_out("resetref");
    ok(o.find("초기화")!=std::string::npos,"resetref OK");

    printf("--- khhist ---\n");
    o=cmd_out("khhist");
    ok(o.find("이력")!=std::string::npos,"khhist 출력");
