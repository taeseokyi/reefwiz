...
cron으로 하루 3회, 측정 스케줄(05/13/21시) + 1시간30분 뒤에 실행(30 6,14,22 * * *)
— 측정이 보통 1시간30분 안에 끝나서 이 시점이면 완료돼 있음. 변경이 없으면 아무것도
커밋하지 않는다(내용이 같으면 push만 시도 — 이전 실행이 네트워크 문제로 push 실패했을
때 재시도 역할). ※측정이 드물게 1시간30분을 넘기면 그 회차는 부분 데이터로 남고, 다음
측정 회차가 되어야 갈아치워짐(하루 3회 고정이라 그 안에서 재시도는 없음).
