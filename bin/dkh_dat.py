...
실전 데이터에는 이제 구형식 행이 없지만(과거 273행은 2026-08-16 에 1회성 마이그레이션
스크립트로 날짜를 채웠다 — 커밋 bc3e763 참조), 손대지 않은 백업 사본이 마지막 줄에
에러 표식(전부 0)을 갖고 있어도 measure_kh_once 의 에러 래치가 그걸 읽어내야 하므로
관용 파싱은 남긴다. **날짜를 되살리는 추측(회차 근사·git blame 복원·원격 정렬)은
전부 폐기했다** — 날짜를 모르는 행은 소비자들이 창 밖으로 본다.
