...
nine-day instrumented campaign (29 logged measurements across five
...
Re-running the null on the same aged aliquot a day later watched it decay from
$-0.06$ to $-0.22\dkh$ with the electronics untouched, confirming the
attribution in the time domain.
...
fix) reproduces the published behaviour of a commercial counterpart, providing
...
DIY build, we instrument a nine-day campaign, expose a $-0.92\dkh$ raw error,
...
equilibrium and latches in $\sim$2.5\,min---a direct illustration of why
measuring tank-first is faster, and of why the span gate alone (satisfied near
$\sim$300\,s) would have latched the tank early.
...
\caption{Complete measurement campaign (29 runs). ``abs pH'' is the common-mode
...
11& 06-17 21:00 & V4 & 8.113 & 8.071 & $-0.042$ & 7.669 & --- & first V4 + trim-mean firmware; clean plateaus \\
12& 06-18 05:00 & V4 & 7.738 & 7.695 & $-0.043$ & 7.657 & --- & \textbf{decisive:} dawn $-0.043\approx$ evening $-0.042$ despite 0.4 abs-pH swing $\Rightarrow$ \co{}-level independence \\
...
18& 06-20 15:00 & V4+SAN& 7.975 & 7.929 & $-0.046$ & 7.602 & 27.2 & first 5\,L anchor; tank \emph{true} plateau (34$\times$, 21.6\,min) yet $-0.046$ $\Rightarrow$ \textbf{real gap}, not latch artifact \\
...
29& 06-22 09:00 & V4+S,CN& 7.959 & 7.948 & $-0.011$ & 8.226 & 27.4 & \textbf{aged complete null} ($\sim$14\,h, $100\,\mathrm{mL}$ exposed aliquot): null decayed to raw err $-0.22$; tank true plateau 59$\times$ (long aeration drives precipitation) \\
\bottomrule
\end{longtable}
}
...
forcing ref and tank to share one box of air, it makes the common \pco{}
intrinsic rather than coincidental, blocks room-\co{} excursions, and (bonus)
suppresses the evaporation that had been concentrating the open tank aliquot.
The effect was immediate (runs \#14/\#15): the dawn dip collapsed from
$-0.05$ to $-0.008$; the dawn$\to$noon dKH jump shrank $\sim$10$\times$
(from $+0.29$ open to $-0.03$ sealed); and the plateau time crushed from
...
\subsection{The active anchor and the persistence of a real gap}
...
swapping hoses did not flip the sign (\emph{not} order, warm-up, or fluid path);
and a head-started tank settled to the \emph{same} equilibrium (\emph{not}
under-equilibration). The gap was a property of the \emph{water}.
...
chemistry rather than electronics, an otherwise-perfect null should \emph{decay}
as the sample ages. We tested this by re-running the complete null on the same
$100\,\mathrm{mL}$ aliquot the next day, without touching the instrument
(Fig.~\ref{fig:degrade}, run \#29). The raw error---$-0.061$ and $-0.014\dkh$ at
0 and 2\,h---fell to $-0.222\dkh$ at $\sim$14\,h ($\dpH=-0.011$; tank pH
$7.957\!\to\!7.948$ while the larger reference held at $7.959$). The drop is well
beyond the $\pm0.08\dkh$ precision and was reached at a verified true plateau
(tank 59 readings), so it is genuine alkalinity loss, not a latching artifact.
The small, exposed aliquot and the unusually long aeration---which raises
carbonate saturation and thus drives \ce{CaCO3} precipitation \emph{during} the
measurement---are exactly the worst case for the $-0.86\dkh$ mechanism, and the
null broke down accordingly while the instrument offset floor stayed at its fresh
value. This is the time-domain confirmation of the error decomposition, and a
practical caveat: an anomalously long plateau on a small sample is itself a
degradation warning.
...
\node[anchor=south west,font=\scriptsize,green!40!black] at (axis cs:-0.8,-0.245) {$\pm0.25\dkh$ goal limit};
...
\node[anchor=north east,font=\scriptsize,blue!45!black,align=right] at (axis cs:13.6,-0.10)
  {$100\,$mL aliquot,\\ $\sim$14\,h, long aeration};
\end{axis}
\end{tikzpicture}
\caption{Time-resolved breakdown of the complete null. With reference water in
\emph{both} vessels (water difference zero by construction), the raw error stays
within the $\pm0.08\dkh$ precision (error bars) and the $\pm0.25\dkh$ goal for the
first hours, then falls to $-0.22\dkh$ after the small ($100\,\mathrm{mL}$),
exposed tank aliquot has aged $\sim$14\,h---a long plateau-following run
(59 readings) further driving \ce{CaCO3} precipitation during aeration. The
electronics are unchanged; only the sample alkalinity drifts, directly confirming
that the dominant error term is sample chemistry (cf.\ Fig.~\ref{fig:budget}).}
\label{fig:degrade}
\end{figure}
...
\section{Discussion}
% =====================================================================
\paragraph{What the diagnosis bought.} The instinct on seeing a stable
$\sim-0.8\dkh$ offset is to zero-point calibrate. Resisting that---on the
...
have baked a sample artifact into the instrument and masked the genuine
$-0.06\dkh$ floor.
...
Notably, commercial units manage by calibration and cleaning and do not pursue a
true equilibrium plateau; our plateau-following step goes one further.
...
\bibitem{repo} nowforyou (REEF FAMILY), ``ReefKeeper: open differential-pH
aeration alkalinity monitor (firmware, host scripts, measurement ledger),''
project repository, \url{https://github.com/taeseokyi/reefkeeper}.
