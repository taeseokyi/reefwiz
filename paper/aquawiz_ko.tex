...
(5개 펌웨어/방법 버전에 걸친 30회 측정)에서 무보정 오차 $-0.92\dkh$가 지속적으로
...
에 걸쳐 널을 재측정하니 전자장치를 건드리지 않았는데도 널이 0에서 멀어지며 연속 측정에서
$-0.22$, 이어 $+0.22\dkh$로 드리프트하는 것을 관찰하여, 이 귀속을 시간 영역에서 확증하였다. 따라서 본 장치는 영점
...
\emph{임시 보관}할 수 있어 위상 간 지연을 최소화한다(\ref{sec:methods}절). 가동 시간은
...
5\,L 동시 폭기).
\item \emph{전이:} \texttt{m2b}로 tank 분취액을 홀딩에 임시 보관; \texttt{m4f}로 참조수를
챔버에 도입.
\item \emph{위상 B:} \texttt{ron}; 평탄까지 \texttt{ref} 측정(이미 평형 근처라 빠름).
\item \texttt{calkh}; 이후 정리: \texttt{m4b}(참조수 5\,L 회수), \texttt{m1b}(임시 보관된
...
게이트만으로는 수렴으로 오인하는 느린 단조 지수 꼬리를 배제한다. 더 긴 net 이력 구간
(8 대 4; ``B1'')은 tank가 일찍 등반을 멈추는 조기 확정에 대한 해결책이었다. 위상별
...
\texttt{FLAT\_NET\_N}   & 8 & net 이력 구간(드리프트 게이트; B1) \\
...
드리프트가 1\,mpH로 떨어지는 $\sim$624\,s에 확정) 붙잡는다. 반면 tank 위상 내내
5\,L에서 동시 폭기된 참조수는 이미 평형 상태라 $\sim$2.6\,min에 확정된다---tank를
먼저 측정하는 것이 빠른 이유이자, span 게이트만으로는($\sim$546\,s 부근에서 충족)
tank를 조기 확정했을 이유를 직접 보여 준다.
...
\node[anchor=west,font=\scriptsize,blue!50!black] at (axis cs:430,7.843) {net 게이트 확정 @624\,s};
...
$\le 1$\,mpH)이 충족될 때만 확정된다; 동시폭기된 참조수(점선)는 이미 평형 근처라
빠르게 확정된다. 두 계열의 pH가 다른 것은 기저 시료가 다르기 때문이며(이 실행은 널
...
\caption{전체 측정 캠페인(30회). ``abs pH''는 공통모드 수준(하루 안에서만 실내 \co{}
프록시). 구성 약어: S=밀폐 헤드스페이스, A=5\,L 능동 앵커, N=net 게이트, B1=net 이력 구간 8.
...
12& 06-18 05:00 & V4 & 7.738 & 7.695 & $-0.043$ & 7.657 & 28.2 & \textbf{결정적:} 새벽 $-0.043\approx$ 저녁 $-0.042$, abs pH 0.4 스윙에도 $\Rightarrow$ \co{}-수준 무관 \\
13& 06-18 13:00 & V4 & 8.081 & 8.055 & $-0.026$ & 7.950 & 28.3 & \#11과 같은 저 \co{}인데 $-0.026$; dKH $+0.29$ 점프 $\Rightarrow$ baseline 비상수(증발, 개방 200\,mL 5일째) \\
14& 06-19 05:00 & V4+S & 7.976 & 7.968 & $-0.008$ & 8.283 & 28.1 & \textbf{밀폐 검증\,\textcircled{1}}: 새벽 강하 붕괴 $-0.05\!\to\!-0.008$; 평탄 33$\to$4 읽기 \\
...
17& 06-20 05:00 & V4+S & 7.922 & 7.871 & $-0.051$ & 7.521 & 27.7 & 새벽 강하 재출현; tank 11$\times$ 단조=조기 확정(진짜 평탄 아님); 냉방이 반응속도 둔화 \\
18& 06-20 15:00 & V4+SAN& 7.975 & 7.929 & $-0.046$ & 7.602 & 27.1 & 첫 5\,L 앵커; tank \emph{진짜} 평탄(34$\times$, 21.6\,min)인데도 $-0.046$ $\Rightarrow$ \textbf{실제 격차}, 확정 아티팩트 아님 \\
...
20& 06-20 18:57 & V4+S,2$\times$& 7.975 & 7.931 & $-0.044$ & 7.636 & 26.7 & ref-먼저 + 호스 교체; 격차 지속 $\Rightarrow$ 순서/경로/미평형 모두 기각 \\
21& 06-20 19:50 & V4+S & 7.975 & 7.926 & $-0.049$ & 7.539 & 26.9 & 재현 \\
22& 06-20 20:29 & V4+SA& 7.977 & 7.921 & $-0.056$ & 7.426 & 26.9 & tank-먼저+임시 보관; ref 역대 최속(4$\times$); tank 조기 확정(7.921) $\Rightarrow$ B1 동기 \\
23& 06-20 21:00 & V4+SA,B1& 7.979 & 7.926 & $-0.053$ & 7.474 & 27.0 & \textbf{B1 성공}: tank 25$\times$/945\,s 진짜 평탄; \#22가 $\sim$5\,mpH 미달이었음 확인; 격차(7번째) \\
24& 06-21 05:00 & V4+SA,B1& 7.974 & 7.928 & $-0.046$ & 7.594 & 26.7 & 새벽이 저녁 군집과 일치 $\Rightarrow$ 시간대 의존성 배제(격차, 8번째) \\
...
29& 06-22 09:00 & V4+S,CN& 7.959 & 7.948 & $-0.011$ & 8.226 & 27.4 & \textbf{노화 완전 널}($\sim$14\,h, $100\,\mathrm{mL}$ 노출 분취액): 널이 원시오차 $-0.22$로 붕괴; tank 진짜 평탄 59$\times$(긴 폭기가 석출 유발, KH$\downarrow$) \\
30& 06-22 13:00 & V4+S,CN& 7.950 & 7.961 & $+0.011$ & 8.664 & 27.6 & \textbf{노화 널 반전}($\sim$18\,h): 짧은 평탄(tank 8$\times$), $\dpH>0$, dKH $8.664$(원시오차 $+0.22$) $\Rightarrow$ 증발 농축(KH$\uparrow$)이 석출을 추월 \\
\bottomrule
\end{longtable}
}
...
값에서 확정되었다. 이 \emph{위상 간 드리프트}는 대략
\begin{equation}
...
공통 공기 부피를 공유하도록 함으로써, 공통 \pco{}를 우연이 아닌 본질로 만들고, 실내
\co{} 변동을 차단하며, 추가로 개방 tank 분취액을 농축하던 증발을 억제한다. 효과는
즉각적이었다(\#14/\#15): 새벽 강하가 $-0.05$에서 $-0.008$로 붕괴; 새벽$\to$낮 dKH
점프가 $\sim$10$\times$ 축소(개방 $+0.29$ $\to$ 밀폐 $-0.03$); 평탄 시간이 $\sim$33
읽기($\sim$50\,min)에서 $\sim$4($\sim$5\,min)로 단축---이것이 다시
식~\eqref{eq:drift}의 $\Delta t$를 붕괴시켜 위상 간 드리프트를 구조적으로 제거한다.
무보정 오차는 $-0.78$에서 $\sim-0.18\dkh$로 떨어졌다.

\subsection{능동 앵커와 실제 격차의 지속}
...
평탄에 도달했을 때조차 $\sim$$-0.046$--$-0.053$ 격차가 \emph{지속}되었다(\#18: 34
읽기, 21.6\,min). 이어진 통제실험 쌍(\#19/\#20)이 네 가지 장치 측 설명을 차례로
기각했다: 폭기를 2배로 하면 등반 시간은 반감했으나 \dpH{}는 불변(반응속도 \emph{아님});
측정 순서를 반전하고 호스를 교체해도 부호가 반전되지 않음(순서/예열/유로 \emph{아님});
더 높은 초기 pH에서 출발한 tank가 \emph{같은} 평형에 안착(미평형 \emph{아님}). 격차는 \emph{물}의
성질이었다.
...
\subsection{널의 시간 의존적 양방향 드리프트}
이 분해는 반증 가능한 예측을 낳는다: 대부분의 오차가 전자장치가 아니라 시료 화학이라면,
완벽해야 할 널이 시료가 노화될수록 지배 화학이 정하는 방향으로 \emph{드리프트}해야 한다.
우리는 같은 $100\,\mathrm{mL}$ 분취액으로 다음 날에 걸쳐 완전 널을 재측정하여(장치는
손대지 않음) 이를 시험했다(그림~\ref{fig:degrade}, \#29--30). 원시 오차는 0\,h와
2\,h에 $-0.061$, $-0.014\dkh$였다가 $\sim$14\,h에 $-0.222\dkh$로 먼저 떨어진 뒤
$\sim$18\,h에 $+0.216\dkh$로 \emph{반전}되었다; 두 극단 모두 검증된 진짜 평탄(tank
59회ㆍ8회)에서 도달했으므로 각각 진짜이지 확정 아티팩트가 아니다. 둘은 한 취약점의 반대
부호다: 긴 폭기(\#29)는 탄산칼슘 과포화를 높여 \ce{CaCO3} 석출을 일으켜 알칼리니티를
낮추고(KH$\downarrow$), 반대로 소부피ㆍ반복 개봉 분취액을 오래 노출한 뒤의 짧은
측정(\#30)은 증발 농축이 이기게 한다(KH$\uparrow$). 어느 쪽이든 소부피 노출 분취액은
$\sim\pm0.2\dkh$ 배회하는 반면 계측 오프셋 바닥은 신선 값을 유지한다. 이는 오차 분해의
시간 영역 확증이자 실무 경고다: 작고 노화된 노출 분취액은 \emph{양방향}으로 불안정하므로
시료는 신선하고 충분한 부피여야 한다.
...
\draw[green!45!black,dashed] (axis cs:-1,-0.25) -- (axis cs:19.5,-0.25);
\node[anchor=east,font=\scriptsize,green!40!black] at (axis cs:19.4,0.29) {$\pm0.25\dkh$ 목표};
...
\node[font=\scriptsize,blue!45!black,align=center] at (axis cs:10.6,-0.135) {석출\\KH$\downarrow$};
\node[font=\scriptsize,blue!45!black,align=center] at (axis cs:15.2,0.125) {증발\\KH$\uparrow$};
\end{axis}
\end{tikzpicture}
\caption{완전 널의 시간 의존적 \emph{양방향} 드리프트. \emph{양} 용기에 참조수를 넣어
(물 차이 구성상 0) 원시 오차는 처음 몇 시간 동안 $\pm0.08\dkh$ 정밀도(오차 막대)와
$\pm0.25\dkh$ 목표 안에 머물다가, 소부피($100\,\mathrm{mL}$)ㆍ노출 tank 분취액이
노화되며 배회한다: 긴 폭기는 \ce{CaCO3} 석출을 일으키고(KH$\downarrow$, $\sim$14\,h에
$-0.22\dkh$), 오랜 노출은 증발 농축이 이기게 한다(KH$\uparrow$, $\sim$18\,h에
$+0.22\dkh$). 전자장치는 그대로이고 시료 알칼리니티만 양방향으로 드리프트하므로, 지배
오차항이 시료 화학임을 직접 확증한다(그림~\ref{fig:budget} 참조).}
\label{fig:degrade}
\end{figure}
...
\section{고찰}
% =====================================================================
\paragraph{오차 분해의 함의.} 안정된 $\sim-0.8\dkh$ 오프셋을 보면 영점 보정하려는
...
\bibitem{repo} nowforyou (REEF FAMILY), ``ReefWiz: 개방형 차동 pH 폭기
알칼리니티 모니터(펌웨어, 호스트 스크립트, 측정 대장),'' 프로젝트 저장소,
\url{https://github.com/taeseokyi/reefwiz}.
